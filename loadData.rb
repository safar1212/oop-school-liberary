...
class LoadData
    def self.load_books
      ...
      record = JSON.parse(File.read('./data/books.json'))
      record.each { |record| books << Book.new(record['title'], record['author']) }
      books
    end
    ...
    def self.load_people
      ...
      record = JSON.parse(File.read('./data/people.json'))
      record.each do |record|
        person = if record['type'] == 'Student'
                   Student.new(record['name'],
                               nil,
                               record['age'],
